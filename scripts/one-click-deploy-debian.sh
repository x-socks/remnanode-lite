...
XRAY_START_TIMEOUT="${XRAY_START_TIMEOUT:-50}"
...
print_intro() {
    cat >&2 <<'EOF'
Remnanode one-click install for Debian

This installer will:
- install or verify Node.js 24, curl, tar, unzip, procps, supervisor, and Xray
- download the selected runtime bundle from GitHub Releases
- install the bare-metal layout under /opt/remnanode
- write the local systemd, supervisord, and env files directly on the host
- keep Xray under a minimal supervisor control plane for upstream compatibility
- write /etc/remnanode/remnanode.env
- enable and start the systemd remnanode service
...
run_preflight() {
    ...
    if [ -f /etc/debian_version ]; then
        ...
        warn "host is not reporting Debian via /etc/debian_version"
    fi
...
if systemctl is-active --quiet remnanode.service; then
    systemctl restart remnanode.service
else
    systemctl start remnanode.service
fi
...
journalctl --no-pager -u remnanode.service -n 50 || true
